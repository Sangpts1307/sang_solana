...
pub struct InvestToken<'info> {
    ...
    /// CHECK: Tài khoản Global State của Staking App (MỚI)
    #[account(mut)]
...
impl<'info> InvestToken<'info> {
    pub fn process(ctx: Context<InvestToken>, amount: u64, is_stake: bool) -> Result<()> {
        ...
        cpi::stake_token(
            CpiContext::new_with_signer(
                ...
                cpi::accounts::StakeToken {
                    global_state: ctx.accounts.staking_global_state.to_account_info(), // Thêm global_state vào đây
        ...
        msg!("InvestToken CPI (Share-based) successful. Action: {}", if is_stake { "Stake" } else { "Unstake" });
        Ok(())
    }
}
